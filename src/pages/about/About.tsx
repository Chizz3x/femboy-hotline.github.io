...
const About = () => {
  const barbie: React.MouseEventHandler<HTMLSpanElement> = (event) => {
    const letters = event.currentTarget.children.length ? [...event.currentTarget.children].map(child => child.innerHTML) : event.currentTarget.innerHTML.split("");
    event.currentTarget.innerHTML = letters.map((letter, index) => `<span delay="${index / 100}" style="animation: pinkWave 1s ease-in-out ${index / 100}s 5">${letter === " " ? "&nbsp;" : letter}</span>`).join("");
  };

  return <AboutStyle>
    <h2>About Femboy Hotline</h2>
    <p>Welcome to Femboy Hotline, the hottest online destination for all things femboy-related! 🌟</p>
    ...
    <p>On our website, you&apos;ll find a monument of femboy existence, various random pages with femboy related stuff and so on... Whether you are a femboy or not you can meme around social platforms with this glorious link to spread the femboyness, and just in general laugh it off.</p>
...
const AboutStyle = styled.div.attrs(props => ({ delay: (props as any).delay }))`
	flex-shrink: 0;
	flex-grow: 1;
	padding: 20px 50px;
	.barbie {
