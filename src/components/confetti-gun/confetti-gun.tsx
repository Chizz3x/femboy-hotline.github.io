...
const ConfettiItem = (
	props: NConfettiGun.IConfettiItemProps,
) => {
	const { acceleration: accelerationInit, id } =
		props;
	const { x = Math.random() * 15, y = 1 } =
		accelerationInit || {};

	const shouldDelete = React.useRef(false);
	const position = React.useRef<
		NConfettiGun.IConfettiItemProps['acceleration']
	>({
		x: 0,
		y: 0,
	});
	...
	const lastFrameId = React.useRef<number | null>(
		null,
	);
	...
	const animate: FrameRequestCallback = (t) => {
		...
		if (itemRef.current) {
			...
			if ((velocity.current?.x || 0) < -50)
				velocity.current.x = -50;
			if ((velocity.current?.x || 0) > 50)
				velocity.current.x = 50;
	...
	return (
		<motion.div
			...
			className={
				shouldDelete.current ? 'delete' : ''
			}
		>
			<StarIcon sx={{ fill: color }} />
		</motion.div>
	);
};

const ConfettiGun = (
	props: NConfettiGun.IProps,
) => {
	const {
		state,
		count = 50,
	...
	const confettiItems = React.useRef<
		{ el: typeof ConfettiItem; id: string }[]
	>([]);
	...
	const el = (
		<ConfettiGunStyle
			ref={containerRef}
			className={['confetti-gun', className].join(
				' ',
			)}
			style={{ ...style }}
		>
	...
	return portal
		? ReactDOM.createPortal(el, portal)
		: el;
};
